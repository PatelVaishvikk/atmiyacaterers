...
const tiffinPlans = [
  { 
    plan: "Mini", 
    category: "Light Meal", 
    items: ["8 oz Sabji", "8 oz Dal", "8 oz Rice", "4 Roti"], 
    dailyPrice: 9.00, 
    monthlyPrice: 190.00, 
    popular: false 
  },
  { 
    ...
    items: ["12 oz Sabji", "8 oz Dal", "8 oz Rice", "5 Roti"], 
    dailyPrice: 10.00, 
    monthlyPrice: 210.00, 
    popular: true 
  },
  { 
    ...
    items: ["12 oz Sabji", "12 oz Dal", "12 oz Rice", "5 Roti"], 
    dailyPrice: 11.00, 
    monthlyPrice: 230.00, 
    popular: false 
  }
...
const addOns = [
  { name: "Extra Rotis", description: "3 additional rotis", price: 2.00 }
];
...
export default function TiffinPlans() {
  ...
  const getCurrentPrice = (plan, subscription, addOns = []) => {
    if (!plan) return 0;
    const basePrice = subscription === "single" ? plan.dailyPrice : plan.monthlyPrice;
    const addOnPrice = addOns.reduce((sum, addOn) => sum + addOn.price, 0);
    const multiplier = subscription === "monthly" ? 22 : 1; // Assuming 22 days per month for add-ons
  ...
  const handleSubmit = () => {
    ...
    const price = getCurrentPrice(selectedPlan, subscriptionType, selectedAddOns);
    ...
    const addOnsText = selectedAddOns.length > 0 
      ? `\n\nADD-ONS:\n${selectedAddOns.map(addOn => `• ${addOn.name} - $${addOn.price.toFixed(2)}`).join('\n')}`
      : '';
  ...
  return (
    <section className="py-16 bg-gray-50">
      ...
      <div className="container mx-auto px-4">
        ...
        {/* Main Plans */}
        <motion.div layout className="grid gap-6 sm:grid-cols-1 lg:grid-cols-3 mb-12">
          {tiffinPlans.map((plan) => (
            <motion.div
              ...
              whileHover={{ scale: 1.02, y: -5 }}
              transition={{ duration: 0.2 }}
              className="bg-white p-6 rounded-xl shadow-lg border-2 border-orange-200 hover:border-orange-300 relative flex flex-col"
            >
              {plan.popular && (
                <div className="absolute -top-2 left-4">
                  <span className="bg-orange-500 text-white px-3 py-1 rounded-full text-xs font-bold">Most Popular</span>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="font-semibold text-blue-600">${plan.monthlyPrice.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <button
                  onClick={() => handleChoosePlan(plan)}
                  className="w-full py-3 px-4 rounded-lg font-semibold transition-colors bg-orange-600 hover:bg-orange-700 text-white"
                >
                  Choose Plan
                </button>
                <p className="text-xs text-gray-500 text-center">Fresh delivery • All vegetarian • Home-style cooking</p>
        ...
        <div className="mb-12">
          <div className="text-center mb-8">
            <h3 className="text-3xl font-bold text-gray-800 mb-3">Customize Your Meal</h3>
            <p className="text-lg text-gray-600">Add these delicious extras to any plan</p>
          </div>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-8 shadow-lg border border-green-100">
              <div className="flex items-center justify-center mb-6">
                ...
                <h4 className="text-xl font-semibold text-gray-800">Available Add-Ons</h4>
              </div>
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {addOns.map((addOn) => (
                  <motion.div 
                    key={addOn.name} 
                    className="bg-white p-6 rounded-xl shadow-md border-2 border-green-200 hover:border-green-300 hover:shadow-lg transition-all duration-200 relative overflow-hidden"
                    whileHover={{ scale: 1.02, y: -2 }}
                  >
                    {/* Decorative corner */}
                    <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-green-400 to-green-500 rounded-bl-3xl opacity-10"></div>
                    ...
                    <div className="flex items-center mb-4">
                      ...
                      <h5 className="font-bold text-gray-800 text-lg">{addOn.name}</h5>
                    </div>
                    
                    {/* Description */}
                    <p className="text-gray-600 mb-4 text-sm leading-relaxed">{addOn.description}</p>
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-100">
                      <p className="text-xs text-gray-500 text-center">Available with any plan</p>
                    </div>
                  </motion.div>
                ))}
              </div>
              ...
              <div className="mt-6 text-center">
                <div className="inline-flex items-center bg-white px-4 py-2 rounded-full border border-green-200 shadow-sm">
                  <svg className="w-4 h-4 text-green-500 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <span className="text-sm text-gray-600">Add-ons can be selected when placing your order</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg p-6 shadow-md max-w-4xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-600">
              <div>
                <p className="font-medium text-gray-700 mb-2">Delivery Schedule:</p>
                <p>• Monthly plans</p>
                <p>• Single day orders available</p>
                <p>• Fresh meals prepared daily</p>
              </div>
              <div>
                <p className="font-medium text-gray-700 mb-2">Service Area:</p>
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4 overscroll-contain"
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[
                    { key: "single", label: "Single Day", note: "Pay per day", value: selectedPlan?.dailyPrice?.toFixed(2) ?? "0.00", color: "text-orange-600" },
                    { key: "monthly", label: "Monthly Plan", note: "22 days per month", value: selectedPlan?.monthlyPrice?.toFixed(2) ?? "0.00", color: "text-blue-600" }
                  ].map(opt => (
                    <label key={opt.key} className="flex items-start sm:items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      <input
                        type="radio"
              ...
              {/* Add-Ons */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-3">Add-Ons (Optional)</label>
                <div className="space-y-2">
                  {addOns.map((addOn) => (
                    <label key={addOn.name} className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      <input
                        type="checkbox"
                      ...
                      <div className="font-bold text-sm text-green-600">+${addOn.price.toFixed(2)}</div>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                <div className="text-center">
                  ...
                  <div className="text-xl sm:text-2xl font-bold text-orange-600">
                    ${getCurrentPrice(selectedPlan, subscriptionType, selectedAddOns).toFixed(2)}
                  </div>
                  <div className="text-xs sm:text-sm text-gray-500">{getSubscriptionLabel(subscriptionType)}</div>
                  {selectedAddOns.length > 0 && (
                    <div className="text-xs text-gray-500 mt-1">
                      Includes add-ons: {selectedAddOns.map(a => a.name).join(", ")}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="bg-gray-50 p-3 rounded-md mb-4 text-sm">
              ...
              {orderDetails.addOns.length > 0 && (
                <p><strong>Add-ons:</strong> {orderDetails.addOns.map(a => a.name).join(", ")}</p>
              )}
            </div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Enter 4‑digit code</label>
            <input
          ...
          <div className="bg-white rounded-lg p-5 w-full max-w-2xl max-h-[90vh] overflow-y-auto receipt-container">
            ...
            <div className="bg-orange-50 p-3 rounded-lg mb-4 tight avoid-break">
              ...
              {orderDetails.addOns.length > 0 && (
                <div className="mt-3 pt-3 border-t border-orange-200">
                  ...
                  <ul className="list-disc list-inside ml-2">
                    {orderDetails.addOns.map((addOn, i) => (
                      <li key={i} className="text-sm">{addOn.name} - ${addOn.price.toFixed(2)}</li>
                    ))}
                  </ul>
                </div>
              )}
              ...
              <div className="text-right mt-3">
                <p className="font-bold text-base">Total: ${orderDetails.pricing.amount.toFixed(2)}</p>
              </div>
            </div>
